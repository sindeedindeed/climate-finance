    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        try {
            ...
            const values = [
                ...
                total_cost_usd, gef_grant, cofinancing, wash_finance,
                wash_finance_percent, beneficiaries, objectives, submitter_email,
                agency_ids, location_ids, funding_source_ids, focal_area_ids, 
                wash_component ? JSON.stringify(wash_component) : null
            ];
            
            const result = await client.query(insertPendingProjectQuery, values);
        ...
        return new Promise((resolve, reject) => {
            ...
            const values = [
                ...
                total_cost_usd, gef_grant, cofinancing, wash_finance,
                wash_finance_percent, beneficiaries, objectives, submitter_email,
                JSON.stringify(agency_ids), JSON.stringify(location_ids), 
                JSON.stringify(funding_source_ids), JSON.stringify(focal_area_ids), 
                wash_component ? JSON.stringify(wash_component) : null
            ];
...
PendingProject.approveProject = async (pendingId) => {
    if (process.env.NODE_ENV === 'production') {
        ...
        try {
            ...
            const projectValues = [
                project_id, pendingProject.title, pendingProject.type, pendingProject.sector, 
                pendingProject.division, pendingProject.status, pendingProject.approval_fy, 
            ...
            if (pendingProject.wash_component) {
                const washData = JSON.parse(pendingProject.wash_component);
                ...
                const washQuery = `
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                `;
                await client.query(washQuery, [project_id, presence, water_supply_percent, sanitation_percent, public_admin_percent]);
            }

            // Insert relationships if they exist
            if (pendingProject.agency_ids && pendingProject.agency_ids.length > 0) {
            ...
            if (pendingProject.location_ids && pendingProject.location_ids.length > 0) {
            ...
            if (pendingProject.funding_source_ids && pendingProject.funding_source_ids.length > 0) {
            ...
            if (pendingProject.focal_area_ids && pendingProject.focal_area_ids.length > 0) {
